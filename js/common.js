...
$("#navigation, #btn").on("click","a", function (event) {
    ...
    //Маска ввода для поля input (phone) 
...
//Кординати
var latitude = 50.429982,
		longitude = 30.454893,
		map_zoom = 18;

	//Адрес до іконки з маркером
	var marker_url = 'wp-content/themes/zpcthemes/img/icon-location.png';
    ...
    $("#form").submit(function() {
        $.ajax({
            type: "POST",
            url: "wp-content/themes/zpcthemes/mail_call.php",
            data: $(this).serialize()
        }).done(function() {
            $(this).find("input").val("");
	...
	//Отправка форми заказа (order)
    $("#form_order").submit(function() {
        var th = $(this);
        var data = th.serialize();
        $.ajax({
            type: "POST",
            url: "wp-content/themes/zpcthemes/mail_order.php",
            beforeSend: function(data) { 
		    th.find('input[type="submit"]').attr('disabled', 'disabled'); 
		    },
        }).done(function() {
            $(this).find("input").val("");
            alert("Спасибо за заказ послуги!");
            setTimeout(function() {
            th.find('input[type="submit"]').prop('disabled', false); 
				$("form").trigger("reset");
				$.magnificPopup.close();
